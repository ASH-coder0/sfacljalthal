...
import { getSingleNotice } from '@/redux/slices/noticeSlice';
...
const SingleNotice = () => {
  ...
  const publishDate = singleNotice?.publishDate && new Date(singleNotice?.publishDate).toISOString().split('T')[0];

  const createdAt = singleNotice?.createdAt && new Date(singleNotice?.createdAt).toISOString().split('T')[0];
  ...
  return (
    <div className='flex flex-row justify-between px-2 md:px-8 lg:px-20 py-2 pt-10 pb-20 '>
      <article className="md:col-span-2 w-full format format-sm sm:format-base lg:format-lg format-blue">
        ...
        {singleNotice?.featuredImage && <figure className="lg:py-10 py-4 max-h-[90vh] max-w-[90vw] flex justify-center px-10">
          <CustomImage src={singleNotice?.featuredImage} className="object-fill max-w-full min-w-[300px] min-h-[300px]" />
        </figure>
        }
        <div
          ...
          dangerouslySetInnerHTML={{ __html: singleNotice?.content }}
        ></div>


        <p className="text-base text-gray-500">
          <time title="">
            Notice published: {publishDate}
          </time>
        </p>
      </article>
    </div>
