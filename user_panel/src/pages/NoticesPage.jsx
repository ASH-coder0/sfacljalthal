...
import { FaBell, FaCalendarAlt, FaUser, FaArrowLeft, FaArrowRight, FaSearch } from 'react-icons/fa';
...
export function NoticesPage() {
  ...
  return (
    <div>
      <PageBanner title="Notice Board" subtitle="Latest announcements and official notices" breadcrumb="Home › Notice Board" />
                ...
                >
                  <div className="w-16 h-16 rounded-lg overflow-hidden flex-shrink-0 bg-primary-100">
                    {notice.featuredImage ? (
                      <img src={getImageUrl(notice.featuredImage)} alt="" className="w-full h-full object-cover" />
                    ) : (
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap items-center gap-3 text-xs text-gray-400">
                      <span className="flex items-center gap-1"><FaCalendarAlt className="text-primary-400" />{formatDate(notice.publishDate || notice.createdAt)}</span>
                      {notice.author && <span className="flex items-center gap-1"><FaUser className="text-primary-400" />{notice.author}</span>}
                    </div>
                  </div>
  ...
  return (
    <div>
      <div className="bg-gradient-to-r from-primary-900 to-primary-700 py-8 px-4">
        <div className="max-w-4xl mx-auto">
          <Link to="/notices" className="inline-flex items-center gap-2 text-primary-200 hover:text-white text-sm mb-4 transition-colors">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
              {notice.featuredImage && (
                <div className="h-56 overflow-hidden">
                  <img src={getImageUrl(notice.featuredImage)} alt={notice.title} className="w-full h-full object-cover" />
                </div>
              )}
              <div className="p-8">
                <div className="flex flex-wrap gap-4 text-sm text-gray-400 mb-6 pb-6 border-b border-gray-100">
                  <span className="flex items-center gap-1.5"><FaCalendarAlt className="text-primary-400" />{formatFullDate(notice.publishDate || notice.createdAt)}</span>
                  {notice.author && <span className="flex items-center gap-1.5"><FaUser className="text-primary-400" />{notice.author}</span>}
                </div>
                {notice.content ? (
                  <div
